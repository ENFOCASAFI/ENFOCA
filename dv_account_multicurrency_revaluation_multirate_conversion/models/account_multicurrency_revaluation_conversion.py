...
class ResCurrencyRate(models.Model):
    ...
    currency_rate_type = fields.Selection(related='currency_id.rate_type', store=True)
    currency_display_name = fields.Char(related='currency_id.display_name', store=True)
class MulticurrencyRevaluationReport(models.Model):
    ...
    def _get_options(self, previous_options=None):
        ...
        usd_currencies = []
        for currency_id in options['currency_rates']:
            if options['currency_rates'][currency_id]['currency_name'] == 'USD':
                usd_currencies.append(currency_id)
        options['usd_currencies'] = usd_currencies
    ...
    def _get_sql(self):
        ...
        return """
            SELECT {move_line_fields},
                   ...
                   aml.balance * currency_rate.rate                     AS report_balance,
                   aml.amount_currency / custom_currency_table.rate     AS report_amount_currency_current,
                   aml.amount_currency / custom_currency_table.rate - aml.balance * currency_rate.rate AS report_adjustment,
            ...
            JOIN res_currency_rate currency_rate ON currency_rate.currency_name = 'USD' AND currency_rate.currency_rate_type = currency.rate_type AND currency_rate.name = (CASE WHEN move.invoice_date IS NOT NULL THEN move.invoice_date ELSE aml.date END)
            ...
            SELECT {move_line_fields},
                   ...
                   CASE WHEN aml.id = part.credit_move_id THEN -part.debit_amount_currency ELSE -part.credit_amount_currency
                   END / custom_currency_table.rate - aml.balance * currency_rate.rate     AS report_adjustment,
                   currency_rate.currency_id                            AS report_currency_id,
                   account.code                                         AS account_code,
                   account.name                                         AS account_name,
                   currency_rate.currency_display_name                  AS currency_code,
            ...
            JOIN {custom_currency_table} ON custom_currency_table.currency_id = account.multirate_conversion_currency_id
            JOIN res_currency_rate currency_rate ON currency_rate.currency_name = 'USD' AND currency_rate.currency_rate_type = currency.rate_type AND currency_rate.name = (CASE WHEN move.invoice_date IS NOT NULL THEN move.invoice_date ELSE aml.date END)
            WHERE (((account.currency_id = aml.company_currency_id OR account.currency_id IS NULL) AND (account.internal_type IN ('receivable', 'payable') AND aml.currency_id = aml.company_currency_id)) AND account.currency_multirate_conversion_affected = TRUE)
        """.format(
    ...
    def _format_report_currency_id_line(self, res, value_dict, options):
        res['name'] = '{comp_cur} (1 {comp_cur} = {rate:.3} {for_cur})'.format(
