...
class BalanceRecordGroupPeriodo(models.Model):
	_name = 'balance.record.group.periodo'
	_auto = False
	_description = 'Campos para reporte de balance por periodos'
	...
	def init(self):
		tools.drop_view_if_exists(self._cr, 'balance_record_group_periodo')
		self._cr.execute("""
            CREATE OR REPLACE VIEW balance_record_group_periodo AS (
				...
				FROM (                        
						...
						FROM 
						 ...
						 ON 
						 	a.account_id = b.id
						 WHERE 
						 	a.account_for_balance_comp= 1 AND a.parent_state = 'posted' 
						GROUP BY
							a.account_id,
							b.code,
							b.name,
