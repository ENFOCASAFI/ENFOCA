...
class BalanceRecordGroupDolar(models.Model):
	...
	_auto = False
	_description = 'Campos para reporte de balance dólares'
	...
	credit_amount = fields.Float('Credito Dólar')
	debit_amount = fields.Float('Debito Dólar')
	...
	def init(self):
		tools.drop_view_if_exists(self._cr, 'balance_record_group_dolar')
		self._cr.execute("""
            CREATE OR REPLACE VIEW balance_record_group_dolar AS (
				...
				FROM (                        
						SELECT 
							...
							sum(a.credit_d) as credit_amount,
							sum(a.debit_d) as debit_amount,							
							CASE WHEN sum(a.debit_d)-sum(a.credit_d) > 0 THEN sum(a.debit_d)-sum(a.credit_d) ELSE 0 END as deudor,
							CASE WHEN sum(a.debit_d)-sum(a.credit_d) < 0 THEN (sum(a.debit_d)-sum(a.credit_d))*(-1) ELSE 0 END as acreedor,
							CASE WHEN substring(b.code, 1, 1) in ('1','2','3','4','5') THEN (CASE WHEN sum(a.debit_d)-sum(a.credit_d) > 0 THEN sum(a.debit_d)-sum(a.credit_d) ELSE 0 END) ELSE 0 END as activo,
							CASE WHEN substring(b.code, 1, 1) in ('1','2','3','4','5') THEN (CASE WHEN sum(a.debit_d)-sum(a.credit_d) < 0 THEN (sum(a.debit_d)-sum(a.credit_d))*(-1) ELSE 0 END) ELSE 0 END as pasivo,
							CASE WHEN (CASE WHEN substring(b.code, 1, 2) = '79' THEN substring(b.code, 1, 2) ELSE substring(b.code, 1, 1) END) in ('6','7','8') THEN (CASE WHEN sum(a.debit_d)-sum(a.credit_d) > 0 THEN sum(a.debit_d)-sum(a.credit_d) ELSE 0 END) ELSE 0 END as perdida,
							CASE WHEN (CASE WHEN substring(b.code, 1, 2) = '79' THEN substring(b.code, 1, 2) ELSE substring(b.code, 1, 1) END) in ('7') THEN (CASE WHEN sum(a.debit_d)-sum(a.credit_d) < 0 THEN (sum(a.debit_d)-sum(a.credit_d))*(-1) ELSE 0 END) ELSE 0 END as ganancia,
							CASE WHEN (CASE WHEN substring(b.code, 1, 2) = '79' THEN substring(b.code, 1, 2) ELSE substring(b.code, 1, 1) END) in ('8','9') THEN (CASE WHEN sum(a.debit_d)-sum(a.credit_d) > 0 THEN sum(a.debit_d)-sum(a.credit_d) ELSE 0 END) ELSE 0 END as perdidan,
							CASE WHEN (CASE WHEN substring(b.code, 1, 2) = '79' THEN substring(b.code, 1, 2) ELSE substring(b.code, 1, 1) END) in ('7') THEN (CASE WHEN sum(a.debit_d)-sum(a.credit_d) < 0 THEN (sum(a.debit_d)-sum(a.credit_d))*(-1) ELSE 0 END) ELSE 0 END as ganancian
